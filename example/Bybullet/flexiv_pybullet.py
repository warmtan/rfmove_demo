...
# rfmove_dir 根目录
rfmove_dir="/home/robotflow/rfmove"
...
class YumiRobotInfo():
    def __init__(self):
        ...
        # 各种配置文件
        self.urdf=rfmove_dir+"/resources/yumi_description/urdf/yumi.urdf"
        self.srdf=rfmove_dir+"/resources/yumi_description/config/yumi.srdf"
        self.kinematics=rfmove_dir+"/resources/yumi_description/config/kinematics.yaml"
        self.joint_limit=rfmove_dir+"/resources/yumi_description/config/joint_limits.yaml"
        self.ompl=rfmove_dir+"/resources/yumi_description/config/ompl_planning.yaml"
...
class YumiBulletRobot: 
    ...
    def get_robot_state(self):
        ...
        for i in range(len(jointstates)):
            jointvalues.append(jointstates[i][0]) #value
        return jointvalues
    ...
    def plan(self,type,targetpose):
        ...
        if type==self.LEFT:
            self.plannerspline.CreateSplineParameterization(targetpose,self.Yumi_Robot_Info.left_arm_group,
                                                                 ...
                                                                 1,1,1)
            self.plannerspline.sample_by_interval(0.02)#0.02
...
if __name__=="__main__":
    ...
    #保存文件到指定地方
    dir_name1 = '/home/robotflow/rfmove/'
    # 判断 D盘下是否存在 test目录，如果不存在该目录，则创建 test目录
    if not os.path.exists(dir_name1):
        os.mkdir(dir_name1)
    fb1 = open(dir_name1 + '1.json', 'w')
    a=np.array(yumiplanner.waypoints)
    a=a[:,12:19]
    a = a.tolist()
    fb1.write(str(a))
    print(yumiplanner.waypoints)
    ...
    #保存文件到指定地方
    dir_name2 = '/home/robotflow/rfmove/'
    # 判断 D盘下是否存在 test目录，如果不存在该目录，则创建 test目录
    if not os.path.exists(dir_name2):
        os.mkdir(dir_name2)
    fb2 = open(dir_name2 + '2.json', 'w')
    b=np.array(yumiplanner.waypoints)
    b=b[:,2:9]
    b = b.tolist()
    fb2.write(str(b))
    print(yumiplanner.waypoints)
